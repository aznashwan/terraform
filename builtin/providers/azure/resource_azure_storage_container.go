...
func resourceAzureStorageContainer() *schema.Resource {
	return &schema.Resource{
		...
		// Update: resourceAzureStorageContainerUpdate,
		...
		Schema: map[string]*schema.Schema{
			...
			"storage_service_key": &schema.Schema{
				Type:        schema.TypeString,
				Required:    true,
				Description: parameterDescriptions["storage_service_key"],
			},
...
func resourceAzureStorageContainerCreate(d *schema.ResourceData, meta interface{}) error {
	storKey := d.Get("storage_service_key").(string)
	storName := d.Get("storage_service_name").(string)
	storageClient, err := storage.NewBasicClient(storName, storKey)
	if err != nil {
		return fmt.Errorf("Failed to create storage account blob client: %s", err)
	}
...
// resourceAzureStorageContainerUpdate does all the necessary API calls update
// the state of the storage container on Azure.
func resourceAzureStorageContainerUpdate(d *schema.ResourceData, meta interface{}) error {
	// NOTE: although empty as most parameters force the creation of a new
	// resource upon change; this is still required if updates to
	// storage_service_key are made.

	// run the ExistsFunc first to ensure the resource's existence.
	_, err := resourceAzureStorageContainerExists(d, meta)
	return err
}
...
func resourceAzureStorageContainerExists(d *schema.ResourceData, meta interface{}) (bool, error) {
	...
	storageClient, err := storage.NewBasicClient(storName, storKey)
	if err != nil {
		return false, fmt.Errorf("Failed to create storage account blob client: %s", err)
	}
	blobClient := storageClient.GetBlobService()
